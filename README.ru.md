...
- **`engine/tts/device.py`** — автодетект CUDA/CPU.
...
- **`local_llm_client.py`** / **`local_env_section.py`** / **`env_setup.py`** — поддержка локальных (офлайн) LLM как альтернативы облачным провайдерам, включая настройку окружения.
...
- **`updater.py`** — проверка/применение обновлений (корректная обработка пробелов в именах файлов через URL-кодирование), самоперезапуск.
